...
object Main extends IOApp:
  ...
  def earliestTime(busIds: Vector[String]): Long =
    ...
    def seive(
      ...
      if currentCheck % currentMod == currentTarget then
        if targets.isEmpty then currentCheck
        else
          val (id, rem) = targets.head
          seive(targets.tail, currentCheck, rem, id, step * currentMod)
      else seive(targets, currentCheck + step, currentTarget, currentMod, step)
